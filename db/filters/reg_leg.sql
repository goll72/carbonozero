...
CREATE OR REPLACE PROCEDURE remove_contradictory_rules() AS $$
    DELETE FROM reg_leg
        WHERE (ent, categoria, nro, ano) IN (
            SELECT a.ent, a.categoria, a.nro, a.ano
                FROM reg_leg AS a
                JOIN reg_leg AS b ON b.categoria = 'multa' AND a.ent = b.ent OR b.ent = substring(a.ent, 1, 2)
                WHERE a.categoria = 'if'
...
CREATE OR REPLACE FUNCTION reg_leg_no_contradictory_rules() RETURNS TRIGGER AS $$
    BEGIN
        ...
        IF new.categoria = 'if' AND (new.meta_if IS NULL OR new.aliq_if IS NULL) THEN
            RETURN NULL;
        END IF;

        IF new.categoria = 'multa' AND (new.lim_multa IS NULL OR new.base_calc_multa IS NULL) THEN
            RETURN NULL;
        END IF;
        
        IF new.categoria = 'if' AND (
            (
